...
# make season and year sample totals, for accounting for the different number of samples per 
inv_szn_samp <- inv %>% group_by(season) %>%
  summarize(season_samples = n())

inv_yr_samp <- inv %>% group_by(year) %>%
  summarize(year_samples = n())

inv_site_samp <- inv %>% group_by(site, sample_type) %>%
  summarize(site_samples = n())
...
inv <- inv %>% left_join(inv_szn_samp, by = "season") %>% 
  ...
  select(date, year, season, site, site_type, sample_type, year_samples, season_samples, site_samples, everything())
...
invTotal <- inv %>% 
  ...
  select(date:site_samples, flies, boaters, mites, crustaceans, worms, mollusks, 
...
```{r}
ggplot(invLong, aes(fill = site_type, y = organisms_L/season_samples, x = season)) + geom_bar(stat = "identity") +
...
ggplot(invLong, aes(fill = sample_type, y = organisms_L/year_samples, x = year(date))) + geom_bar(stat = "identity") +
  ggtitle("Average organisms per sample collected by year") + ylab("organisms/L per sample") + labs(fill = "sample type")
...
```{r}
invTotalLong %>% filter(sample_type == "FB250") %>% 
  ggplot(aes(x = group, y = organisms_L/season_samples, fill = season)) +
  geom_bar(stat = "identity", position = "dodge")

invTotalLong %>% filter(sample_type == "FB250", !group %in% c("crustaceans", "boaters", "worms")) %>% 
  ggplot(aes(x = group, y = organisms_L/season_samples, fill = season)) +
  geom_bar(stat = "identity", position = "dodge")
