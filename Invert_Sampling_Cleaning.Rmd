...
colNoID <- inv[, 10:length(inv)] %>% select_if(colSums(., na.rm = T) != 0) %>% colnames() %>% 
  setdiff(colnames(inv[, 10:length(inv)]), .)  

inv <- inv %>% select(-colNoID)
...
# Add site metadata
inv <- inv %>% left_join(site_index, by = c("site")) %>%
...
# Add FB250 pre-filtration sample volume
  ...
  select(date, year, season, breach_status, 
         ...
         sample_type, filter_method, everything())
```

## Add sample normalization factors
```{r}
# Make season and year sample slough totals
inv_szn_samp <- inv %>% filter(site_type == "slough") %>% group_by(season, sample_type) %>%
  dplyr::summarize(season_samples = n())

inv_yr_samp <- inv %>% filter(site_type == "slough") %>% group_by(year, sample_type) %>%
...
inv <- inv %>% left_join(inv_szn_samp, by = c("season", "sample_type")) %>% 
  left_join(inv_yr_samp, by = c("year", "sample_type")) %>% 
  left_join(inv_site_samp, by = c("site", "sample_type")) %>%
  select(date, year, season, breach_status, 
         site, site_type, site_reserve, site_desc, 
         sample_type, filter_method, season_samples, year_samples, site_samples, everything())
```
...
rm(inv2018csv, inv2019csv, inv2020_22csv, wq2020_22csv, 
   inv2018, inv2019, inv2021, colNoID, 
   inv_site_samp, inv_szn_samp, inv_yr_samp)
```
